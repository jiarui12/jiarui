...
- (void)viewDidLoad {
    ...
    NSString * str=[user objectForKey:@"progerss"];
        if (str) {
            float progress=[str floatValue];
            self.topProgressSlider.value=progress;
                        NSLog(@"---------------------%f",progress);
        }else{
...
-(void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context{
    ...
    if ([keyPath isEqualToString:@"status"]) {
        ...
        if (status == AVPlayerStatusReadyToPlay) {
            ...
            /*  取出上次储存的播放进度   实现断点续播**/
            NSUserDefaults * user=[NSUserDefaults standardUserDefaults];
            NSString * progress=[user objectForKey:@"kpID"];
            
            NSString  * slider=[user objectForKey:progress];
            NSLog(@"***************%@**************",slider);
            if (slider) {
                
                
                double currentTime = floor(_totalMovieDuration * slider.floatValue);
...
[self setMovieParse];
    ...
    if ([keyPath isEqualToString:@"loadedTimeRanges"]) {
        ...
        NSUserDefaults * user=[NSUserDefaults standardUserDefaults];
        NSString * str=[NSString stringWithFormat:@"%f",progressSlider];
        NSLog(@"%@",str);
        
        NSString * progress=[user objectForKey:@"kpID"];
        
        NSLog(@"储存的播放进度为~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~%@",progress);
        
        [user setObject:str forKey:progress];
