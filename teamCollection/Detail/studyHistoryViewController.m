...
@interface studyHistoryViewController ()<UITableViewDelegate,UITableViewDataSource,UIViewControllerPreviewingDelegate>
...
@implementation studyHistoryViewController
- (void)viewDidLoad {
    [super viewDidLoad];
    ...
    self.navigationItem.title=@"学习历史";
...
-(void)loadData{
    ...
    NSString * url=[NSString stringWithFormat:@"%@/BagServer/getStudyHistoryByUserID.mob?token=%@&pageNumber=1",URLDOMAIN,[user objectForKey:@"token"]];
...
-(void)loadDataWithPage:(int)page{
    ...
    manager.responseSerializer=[AFJSONResponseSerializer serializer];
    NSUserDefaults * user=[NSUserDefaults standardUserDefaults];
    NSString * url=[NSString stringWithFormat:@"%@/BagServer/getStudyHistoryByUserID.mob?token=%@&pageNumber=%d",URLDOMAIN,[user objectForKey:@"token"],page];
