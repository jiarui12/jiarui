...
//        _second--;
        ...
        {
            NSString *title = [NSString stringWithFormat:@"验证码已发送（%ds）",_second];
    ...
    if (_timer) {
        if ([_timer respondsToSelector:@selector(isValid)])
        {
            if ([_timer isValid])
            {
                ...
                {
                    [self setTitle:@"验证码已发送（60s）" forState:UIControlStateNormal];
                    [self setTitle:@"验证码已发送（60s）" forState:UIControlStateDisabled];
