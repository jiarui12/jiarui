...
@property (nonatomic,strong)NSString * isCanPingfen;
...
-(instancetype)initWithFrame:(CGRect)frame isCanDian:(BOOL)isCanDian finish:(finishBlock)finish;
-(instancetype)initWithFrame:(CGRect)frame numberOfStars:(NSInteger)numberOfStars rateStyle:(RateStyle)rateStyle isAnination:(BOOL)isAnimation isCanDian:(BOOL)isCanDian finish:(finishBlock)finish;
