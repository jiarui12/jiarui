...
- (void)dealloc {
    ...
    [user setObject:[NSString stringWithFormat:@"%f",_videoCurrent] forKey:[NSString stringWithFormat:@"%@",_videoURL]];
...
- (SRVideoTopBar *)topBar {
    ...
    if (!_topBar) {
        ...
        _topBar.hidden=YES;
        _topBar.backgroundColor=[UIColor colorWithRed:0 green:0 blue:0 alpha:0.42];
...
- (SRVideoBottomBar *)bottomBar {
    ...
    if (!_bottomBar) {
        ...
        _bottomBar.backgroundColor=[UIColor colorWithRed:0 green:0 blue:0 alpha:0.42];
    }
    return _bottomBar;
}
...
- (UIButton *)replayBtn {
    ...
    if (!_replayBtn) {
        ...
        [_replayBtn setImage:[UIImage imageNamed:SRVideoPlayerImageName(@"icon_repeat_video")] forState:UIControlStateNormal];
...
- (void)setupSubViews {
    ...
    [_playerView addSubview:self.replayLabel];
    ...
    if ([keyPath isEqualToString:SRVideoPlayerItemStatusKeyPath]) {
        ...
        switch (playerItem.status) {
            case AVPlayerStatusReadyToPlay:
            {
             ...
             NSString *  current =  [user objectForKey:[NSString stringWithFormat:@"%@",_videoURL]];
                ...
                [self.player pause];
                [self.player seekToTime:CMTimeMakeWithSeconds([current floatValue], NSEC_PER_SEC) completionHandler:^(BOOL finished) {
                    [self.player play];
                    _isManualPaused = NO;
                    _playerState = SRVideoPlayerStatePlaying;
...
- (void)changeToOrientation:(UIInterfaceOrientation)orientation {
    ...
    switch (orientation) {
        ...
        case UIInterfaceOrientationPortraitUpsideDown:
        {
            ...
            SRBrightnessView *lightView = [SRBrightnessView sharedBrightnessView];
            ...
            
        {
            ...
            SRBrightnessView *lightView = [SRBrightnessView sharedBrightnessView];
            [[UIApplication sharedApplication].keyWindow insertSubview:self.playerView belowSubview:lightView];
    ...
    _currentOrientation = orientation;
        ...
        [UIView animateWithDuration:0.5 animations:^{
            [[UIApplication sharedApplication] setStatusBarOrientation:_currentOrientation animated:NO];

            _playerView.transform = [self getTransformWithOrientation:orientation];
            [SRBrightnessView sharedBrightnessView].transform=[self getTransformWithOrientation:orientation];
...
//        [UIDevice setOrientation:UIInterfaceOrientationPortrait];
    ...
    UINavigationController * nav = [self navigationController:_playerView];
