...
- (id)initWithFrame:(CGRect)frame titleName:(NSString *)title {
    ...
    if (self) {
        ...
        //add subView
        self.arrow = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"UnselectedJian"]];
        self.arrow.frame = CGRectMake(self.width - ArrowSide -4*ArrowToRight,(self.height - ArrowSide)/2  , ArrowSide , ArrowSide);
        [self addSubview:self.arrow];
        
        self.titleLabel = [[UILabel alloc] init];
        self.titleLabel.font = [UIFont systemFontOfSize:kScreenWidth*14/375];
...
//        self.titleLabel.frame = CGRectMake(DropDownBoxTitleHorizontalToLeft, 0 ,self.arrow.left - DropDownBoxTitleHorizontalToArrow - DropDownBoxTitleHorizontalToLeft  , self.height);
        ...
        self.line.frame = CGRectMake(self.width - 1.0/scale , 0, 1.0/scale, self.height);
...
- (void)updateTitleState:(BOOL)isSelected {
    if (isSelected) {
        ...
        self.arrow.image = [UIImage imageNamed:@"selectedJian"];
    } else{
        self.titleLabel.textColor = [UIColor blackColor];
        self.arrow.image = [UIImage imageNamed:@"UnselectedJian"];
